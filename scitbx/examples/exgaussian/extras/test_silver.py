...
w_obs = 1./(y_obs)
initial2 = flex.double([100, 800, 10, 87, 2])
initial3 = flex.double([1,1,1,1,1])
...
class lbfgs_biexponential_fit (bevington_silver):
  ...
  def biexponential_func(self):
    X1 = x_obs.as_numpy_array()
    F1 = self.a[0]+self.a[1]*np.exp(-X1/self.x[3])+self.a[2]*np.exp(-X1/self.a[4])
    return F1
...
#    print 'gradients',g[0],g[1], g[2], g[3],g[4]
    return f, g
...
def lbfgs_example(verbose):
  ...
  X0 = x_obs.as_numpy_array()
  Y0 = y_obs.as_numpy_array()
  fit2 = lbfgs_biexponential_fit(x_obs=x_obs,y_obs=y_obs,w_obs=w_obs,initial=initial2)
  plt.plot(X0, Y0, 'o', label='data')
  F1 = fit.biexponential_func()
  F2 = fit2.biexponential_func()
  plt.plot(X0, F1, 'g+', label='fit', linewidth=1) 
#  plt.plot(X0, F2, 'r-', label='fit2', linewidth=1) 
  plt.show()
