...
# http://aria42.com/blog/2014/12/understanding-lbfgs
  ...
  def exgauss_cdf_nparray(self,data,mu,sigma,tau):
    ...
    for x in data:
      u = (x-mu)/tau
      v = sigma/tau
      cdf.append(self.gauss_cdf(u,0,v)-np.exp(-u+0.5*v*v+np.log(self.gauss_cdf(u,v*v,v)+1e-100)))
    return np.array(cdf)

  def exgauss_cdf(self,x, mu, sigma, tau):
    u = (x-mu)/tau
    v = sigma/tau
    return self.gauss_cdf(u,0,v)-np.exp(-u+0.5*v*v+np.log(self.gauss_cdf(u,v*v,v)+1e-100))
  ...
  def target_func_and_grad(self):
    ...
    for i in range(0, self.x_obs.size()):
      ...
      v = sigma/tau
      phi_uv2v = self.gauss_cdf(u, v*v, v) + 1e-100
      z1 = (self.x_obs[i] - mu)/(sigma*np.sqrt(2)) + 1e-100
      z2 = (self.x_obs[i]- mu-(sigma*sigma/tau))/(sigma*np.sqrt(2)) + 1e-100
      exp_1 = np.exp(-z1*z1)/np.sqrt(2*np.pi*sigma*sigma)  + 1e-100
      exp_2 = np.exp(-z2*z2)/np.sqrt(np.pi*phi_uv2v*phi_uv2v) +1e-100
      exp_0 = np.exp(-u+0.5*v*v+np.log(phi_uv2v)) +1e-100
