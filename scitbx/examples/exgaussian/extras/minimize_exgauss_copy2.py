...
#    self.set_cpp_data(x_obs,y_obs,w_obs)
    ...
    self.minimizer = scitbx.lbfgs.run(target_evaluator=self, log = sys.stdout,
...
# http://aria42.com/blog/2014/12/understanding-lbfgs
  ...
  def target_func_and_grad(self):
    ...
    for i in range(0, self.x_obs.size()):
      ...
      z1 = (self.x_obs[i] - mu)/(sigma*np.sqrt(2)) #+ 1e-15
      z2 = (self.x_obs[i]- mu-(sigma*sigma/tau))/(sigma*np.sqrt(2)) #+ 1e-15
      exp_1 = np.exp(-z1*z1)/np.sqrt(2*np.pi*sigma*sigma)  #+ 1e-15
      exp_2 = np.exp(-z2*z2)/np.sqrt(np.pi*phi_uv2v*phi_uv2v) #+1e-15
      exp_0 = np.exp(-u+0.5*v*v+np.log(phi_uv2v)) #+1e-15
...
#    F1 = fit.exgauss_cdf_nparray(X1,-4000.,4000., 25000.)
  else:
